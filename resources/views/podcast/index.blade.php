...
<main class="container mx-auto px-4 py-8 mt-[70px]">
    <h4 class="text-3xl font-bold text-red-600 mb-5">Podcast</h4>
    <div class="flex flex-col lg:flex-row gap-8">

        <!-- Left Column (Main Content) -->
        <div class="w-full lg:w-2/3">
            @if ($podcast->isEmpty())
            <p class="text-center text-gray-500">Tidak ada podcast.</p>
            @else
            <!-- News Grid -->
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                    @foreach($podcast as $item)
                    <a href="{{ route('podcast.show', $item->slug) }}" class="block">
                        <div
